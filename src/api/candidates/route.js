async function handler({ page = 1, search, gender, experience, skills }) {
  ...
  if (skills && skills.length > 0) {
    queryString += ` AND c.id IN (
      SELECT candidate_id 
      FROM candidate_skills 
      WHERE skill_id = ANY($${paramCount})
    )`;
    values.push(skills);
    paramCount++;
  }

  queryString += ` GROUP BY c.id, c.name, c.phone, c.email, c.gender, c.experience, c.created_at`;
